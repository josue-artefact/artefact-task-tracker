  ...
  return (
    <AppShell user={user}>
      ...
      <section className="mb-10 grid grid-cols-1 gap-4 md:grid-cols-3">
        {[
          { label: "Abiertas", value: open.length, accent: "bg-cream-300 text-ink-900" },
          ...
          >
            <div
              className={`flex items-end justify-between rounded-xl px-6 py-7 ${m.accent}`}
            >
              <span className="text-[11px] uppercase tracking-[0.22em] opacity-70">{m.label}</span>
              <span className="font-semibold tracking-tight text-[56px] leading-none">{m.value}</span>
            </div>
          </div>
        ))}
      </section>
      ...
      <section className="space-y-3">
        ...
        {open.length === 0 && (
          <div className="rounded-2xl bg-ink-900/[0.03] p-1.5 ring-1 ring-ink-900/5">
            <div className="rounded-xl bg-cream-100 px-8 py-16 text-center">
              <p className="font-semibold tracking-tight text-2xl text-ink-700">Todo al día.</p>
        ...
        {open.map((t, i) => (
          ...
          >
            <article className="flex flex-col gap-4 rounded-xl bg-cream-100 px-6 py-5 sm:flex-row sm:items-center sm:justify-between">
              <div className="min-w-0 flex-1">
                ...
                <h3 className="mt-1.5 truncate font-semibold tracking-tight text-[22px] leading-snug text-ink-900">
                  {t.title}
                </h3>
              </div>
              ...
              <div className="flex shrink-0 flex-wrap items-center gap-2">
                ...
                <span
                  className="ml-1 flex h-8 w-8 items-center justify-center rounded-full bg-ink-900/[0.05] text-ink-700 transition-all duration-500 ease-[cubic-bezier(0.32,0.72,0,1)] group-hover:bg-cream-300 group-hover:text-ink-900 group-hover:translate-x-0.5"
                >
        ...
        <section className="mt-12 space-y-3">
          ...
          {done.map((t) => (
            <Link
              key={t.id}
              href={`/task/${t.id}`}
              className="group block rounded-xl bg-ink-900/[0.02] px-5 py-4 ring-1 ring-ink-900/[0.04] transition hover:bg-ink-900/[0.04]"
            >
              <div className="flex items-center justify-between">
                <div className="min-w-0">
                  ...
                  <h3 className="truncate font-semibold tracking-tight text-[17px] leading-snug text-ink-500 line-through decoration-ink-300">
                    {t.title}
                  </h3>
                </div>
                <span className="text-[10px] uppercase tracking-[0.2em] text-ink-400">
